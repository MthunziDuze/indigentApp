...
(function () {
    ...
    var app = angular.module('app', ['onsen', 'angular-images-loaded', 'ngMap', 'nvd3', 'ngMessages', 'angularSoap', 'pdf', 'ngSQLite', 'ngCordova']);
    ...
    app.controller("ExampleController", function ($scope, $cordovaSQLite) {
        ...
        if (window.cordova) {
            ...
            //db.execute("CREATE TABLE tt (data)");
            console.log("Android");
        } else {
            ...
            function onDeviceReady() {
                ...
                db.transaction(function (tx) {
                    tx.executeSql('DROP TABLE IF EXISTS accepted');
                    tx.executeSql('CREATE TABLE IF NOT EXISTS accepted (data)');

                    tx.executeSql("INSERT INTO accepted (data) VALUES (?)", [$scope.API], function (tx, res) {
                        console.log("insertId: " + res.insertId + " -- probably 1");
                        console.log("rowsAffected: " + res.rowsAffected + " -- should be 1");

                        tx.executeSql("select count(id) as cnt from accepted;", [], function (tx, res) {
        ...
        //		var query = "INSERT INTO ZZZZZZ(cellNo,preferredEmail,preferredPost,preferredSMS,ownership,ownership,Heaters,Cooking,Radio,WashingMachine,Vaccuming,Geyser,TV,Lights,outerSpace,outerSpace,outerSpace,innerSpace,propertyKind,idNo,title,initials,gender,gender,surname,birthDate,maritalStatus,firstNames,firstNames,personID,postalCode,suburb,town,addressID,addressLine2,addressLine2 ,addressLine2 , addressID , addressLine2 , addressLine1 , imageList , clothingDetail , foodDetail , medicalDetail , imageNameList ,indigentOffice , fieldWorkerArea , fieldWorkerArea ,water ,bondPayment ,other ,medical ,transport ,food ,electricity ,education ,rental ,rental , poorHealthDetails , disability , mentalDefect , amount , uif , accountHolder , previousWorkPension , otherIncome , propertyRenting , oldAgePension ,totalPersonIncome , disabilityPension , homeBusiness , relationship , idNo , title , initials , gender , surname , genderDisplay , birthDate , titleDisplay , firstNames , personID , currentApplicationRefNo , currentApplicationRefNo , WardNo , applicationRefNo , applicationState , healthIndigentClerk , wardCouncillor , financeClerk , applicantKind , applicationType , debtorKind , applicationCCA , applicationCCC , applicationDate , applicationDate , applicationStatus , erfNo , emmAccountNo , personID , status ) VALUES (?, ?, ?, ?, ? , ? , ?, ?, ?, ?, ?, ?, ?, ?, ?, ? , ?, ?, ?,?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?,?,?,?,?,?,?,?,?,?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? , ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        //		$cordovaSQLite.execute(db,query,[$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id,$scope.applicantDetails._id]).then(function(result) {
        ...
        //		var query = "SELECT firstname, lastname FROM indigent WHERE lastname = ?";
...
/*            navigator.geolocation.getCurrentPosition(function (position) {
    ...
    app.controller('indigentController', ['$http', '$scope', '$rootScope', '$sce', 'appConfig', 'loadingMessageService', '$soap', '$q', '$cordovaSQLite', function ($http, $scope, $rootScope, $sce, appConfig, loadingMessageService, $soap, $q, $cordovaSQLite) {
        ...
        $scope.loadAccepted = function () {
            ...
            if (window.cordova) {
                ...
                db.transaction(function (tx) {
                    $scope.acceptedList = [];
                    //select from table accepted
                    var query = "SELECT  * from accepted  ";

                    $cordovaSQLite.execute(db, query, []).then(function (res) {

                        if (res.rows.length > 0) {
                            for (var i = 0; i < res.rows.length; i++) {
                                $scope.acceptedList.push(JSON.parse(res.rows.item(i).objectString));
                            }
                        } else {
            ...
            /*$scope.API = 'https://munipoiapp.herokuapp.com/api/applications/Accepted';

            console.log($scope.API);

            $http.get($scope.API).success(function (data) {
				console.log(data);
				$rootScope.acceptedList = data;
...
*/
        ...
        $scope.loadClosedDetails = function (index, closed) {
            $scope.id = closed._id;
            appNavigator.pushPage('closedDetails.html', {
                id: closed._id,
		...
		$scope.loadDeclinedDetails = function (index, closed) {
            $scope.id = closed._id;
        ...
        $scope.loadDeclined = function () {
            ...
            $scope.API = 'https://munipoiapp.herokuapp.com/api/applications/Declined';

            console.log($scope.API);

            $http.get($scope.API).success(function (data) {
				
                $scope.declinedList = data;
		...
		$scope.loadRejected = function () {
            ...
            $scope.API = 'https://munipoiapp.herokuapp.com/api/applications/Rejected';
		...
		$scope.showAcceptedDetails = function () {
            ...
            if (window.cordova) {
                ...
                db.transaction(function (tx) {
					...
					var query = "SELECT  * from accepted WHERE id='" + page.options.id + "'";
					
                    $cordovaSQLite.execute(db, query, []).then(function (res) {

                        if (res.rows.length > 0) {

                            $scope.applicantDetails = JSON.parse(res.rows.item(0).objectString);
        ...
        $scope.capturePhoto = function (selection) {

            var defer = $q.defer();
            defer.promise.then(function (imageUri) {
        ...
        $scope.loadDocumentInfo = function () {
            ...
            $scope.documentInfoList = [];

            $scope.API = 'https://munipoiapp.herokuapp.com/api/assigments/imagelist/' + $scope.applicationRefNo;

            $http.get($scope.API).success(function (data) {

                if (data.length > 0)
                    $scope.documentInfoList = data[0].indigentApplicationDetails.imageNameList;
        ...
        $scope.viewHouseholdDetails = function (index, household) {
            ...
            appNavigator.pushPage('householdverify.html', {
                ...
                currentIndex: JSON.stringify(index)
            });
        }
        ...
        $scope.loadHouseholdDetails = function () {
            ...
            $scope.currentIndex = JSON.parse(page.options.currentIndex);

        }

        $scope.houseHoldMemberVerified = function (index) {
            console.log(index + $scope.remarks);
        }
        ...
        $scope.saveCanvas = function () {
            ...
            window.localStorage.setItem("applicationSignature", JSON.stringify($scope.signature));
        ...
        $scope.setApplicationAccepted = function () {
            ...
            $http.get($scope.API).success(function (data) {
                ...
                appNavigator.pushPage('accepted.html');
		...
		$scope.setApplicationDeclined = function () {
            ...
            //TODO add this to the config file
			$scope.API = 'http://196.15.242.146:5555/rest/EMMShared/resources/updateFieldWorkerTaskStatus/{"taskStatus":"Declined","applicationId":"' + applicationRefNo + '","reasonForRejection":"' + $scope.reason.name + '"}';
            ...
            $http.get($scope.API).success(function (data) {
                ...
                appNavigator.pushPage('declined.html');
        ...
        $scope.saveApplicationDetails = function () {
            ...
            if (window.cordova) {
                ...
                db.transaction(function (tx) {
                    ...
                    tx.executeSql('CREATE TABLE IF NOT EXISTS accepted (id,status,objectString)');

                    tx.executeSql("INSERT INTO accepted (id,status,objectString) VALUES (?,?,?)", [$scope.applicantDetails._id, $scope.status, JSON.stringify($scope.applicantDetails)], function (tx, res) {
                        console.log("insertId: " + res.insertId + " -- probably 1");
                        console.log("rowsAffected: " + res.rowsAffected + " -- should be 1");
    ...
    app.controller('DocCtrl', ['$http', '$scope', '$rootScope', '$sce', 'appConfig', 'loadingMessageService', '$soap', '$q', function ($http, $scope, $rootScope, $sce, appConfig, loadingMessageService, $soap, $q) {
        ...
        $scope.loadDocumentInfo = function () {
            ...
            $http.get($scope.API).success(function (data) {
                ...
                $scope.pdfData = "data:application/pdf;base64," + data.indigentApplicationDetails.imageList[0];
